...
template <typename R>
struct StringMaker<
    R, typename std::enable_if<is_range<R>::value &&
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                           \
	...
	} while ((void)0, false && static_cast<bool>(!!(__VA_ARGS__)))  // the expression here
...
#define INTERNAL_CATCH_INFO(macroName, log)                                             \
	Catch::ScopedMessage INTERNAL_CATCH_UNIQUE_NAME(scopedMessage)(                       \
	    Catch::MessageBuilder(macroName, CATCH_INTERNAL_LINEINFO, Catch::ResultWas::Info) \
	    << log);
...
class Approx {
 ...
 public:
	...
	Approx& epsilon(T const& newEpsilon) {
		...
		if (epsilonAsDouble < 0 || epsilonAsDouble > 1.0) {
			throw std::domain_error(
			    "Invalid Approx::epsilon: " + Catch::Detail::stringify(epsilonAsDouble) +
	...
	Approx& margin(T const& newMargin) {
		...
		if (marginAsDouble < 0) {
			throw std::domain_error(
			    "Invalid Approx::margin: " + Catch::Detail::stringify(marginAsDouble) +
...
StdString::RegexMatcher Matches(
    std::string const& regex, CaseSensitive::Choice caseSensitivity = CaseSensitive::Yes);
...
void AssertionHandler::complete() {
	...
	if (m_reaction.shouldThrow) throw Catch::TestFailureException();
}
...
void BenchmarkLooper::reportStart() { getResultCapture().benchmarkStarting({m_name}); }
...
// Wraps a token coming from a token stream. These may not directly correspond to strings
// as a single string may encode an option + its argument if the : or = form is used
...
template <typename T>
inline auto convertInto(std::string const& source, std::optional<T>& target)
    -> ParserResult {
	T temp;
	auto result = convertInto(source, temp);
	if (result) target = temp;
	return result;
}
...
#define CATCH_INTERNAL_ERROR(msg) \
	throw CATCH_PREPARE_EXCEPTION(  \
	    std::logic_error, CATCH_INTERNAL_LINEINFO << ": Internal Catch error: " << msg);
...
void seedRng(IConfig const& config) {
	if (config.rngSeed() != 0) std::srand(config.rngSeed());
}
...
IStreamingReporterPtr makeReporter(std::shared_ptr<Config> const& config) {
	...
	if (reporterNames.empty()) return createReporter(CATCH_CONFIG_DEFAULT_REPORTER, config);
...
int Session::runInternal() {
	...
	try {
		...
		// Handle list request
		if (Option<std::size_t> listed = list(config())) return static_cast<int>(*listed);
		...
		return (std::min)(
		    {MaxExitCode, totals.error, static_cast<int>(totals.assertions.failed)});
	} catch (std::exception& ex) {
...
struct StringStreams {
	...
	static auto instance() -> StringStreams& {
		if (!s_instance) s_instance = new StringStreams();
		return *s_instance;
	}
	static void cleanup() {
		delete s_instance;
...
IndexTracker::IndexTracker(NameAndLocation const& nameAndLocation, TrackerContext& ctx,
                           ITracker* parent, int size)
...
void IndexTracker::close() {
	TrackerBase::close();
	if (m_runState == CompletedSuccessfully && m_index < m_size - 1) m_runState = Executing;
}
...
std::string StringMaker<double>::convert(double value) { return fpToString(value, 10); }
 ...
 public:
	...
	void open() {
		if (!m_isOpen) {
			m_isOpen = true;
			*this << RowBreak();
			for (auto const& info : m_columnInfos) *this << info.name << ColumnBreak();
			*this << RowBreak();
...
void ConsoleReporter::benchmarkEnded(BenchmarkStats const& stats) {
	...
	(*m_tablePrinter) << stats.iterations << ColumnBreak() << stats.elapsedTimeInNanoseconds
	                  << ColumnBreak() << average << ColumnBreak();
}
...
void MultipleReporters::noMatchingTestCases(std::string const& spec) {
	for (auto const& reporter : m_reporters) reporter->noMatchingTestCases(spec);
}

void MultipleReporters::benchmarkStarting(BenchmarkInfo const& benchmarkInfo) {
	for (auto const& reporter : m_reporters) reporter->benchmarkStarting(benchmarkInfo);
}
void MultipleReporters::benchmarkEnded(BenchmarkStats const& benchmarkStats) {
	for (auto const& reporter : m_reporters) reporter->benchmarkEnded(benchmarkStats);
}

void MultipleReporters::testRunStarting(TestRunInfo const& testRunInfo) {
	for (auto const& reporter : m_reporters) reporter->testRunStarting(testRunInfo);
}

void MultipleReporters::testGroupStarting(GroupInfo const& groupInfo) {
	for (auto const& reporter : m_reporters) reporter->testGroupStarting(groupInfo);
}

void MultipleReporters::testCaseStarting(TestCaseInfo const& testInfo) {
	for (auto const& reporter : m_reporters) reporter->testCaseStarting(testInfo);
}

void MultipleReporters::sectionStarting(SectionInfo const& sectionInfo) {
	for (auto const& reporter : m_reporters) reporter->sectionStarting(sectionInfo);
}
...
void MultipleReporters::sectionEnded(SectionStats const& sectionStats) {
	for (auto const& reporter : m_reporters) reporter->sectionEnded(sectionStats);
}

void MultipleReporters::testCaseEnded(TestCaseStats const& testCaseStats) {
	for (auto const& reporter : m_reporters) reporter->testCaseEnded(testCaseStats);
}

void MultipleReporters::testGroupEnded(TestGroupStats const& testGroupStats) {
	for (auto const& reporter : m_reporters) reporter->testGroupEnded(testGroupStats);
}

void MultipleReporters::testRunEnded(TestRunStats const& testRunStats) {
	for (auto const& reporter : m_reporters) reporter->testRunEnded(testRunStats);
}

void MultipleReporters::skipTest(TestCaseInfo const& testInfo) {
	for (auto const& reporter : m_reporters) reporter->skipTest(testInfo);
}
...
#define CATCH_CHECK_THROWS_MATCHES(expr, exceptionType, matcher)                      \
	INTERNAL_CATCH_THROWS_MATCHES("CATCH_CHECK_THROWS_MATCHES", exceptionType,          \
	                              Catch::ResultDisposition::ContinueOnFailure, matcher, \
	                              expr)
#endif  // CATCH_CONFIG_DISABLE_MATCHERS
...
#define CATCH_REQUIRE_THAT(arg, matcher)                                               \
	INTERNAL_CHECK_THAT("CATCH_REQUIRE_THAT", matcher, Catch::ResultDisposition::Normal, \
	                    arg)
#endif  // CATCH_CONFIG_DISABLE_MATCHERS
...
#define CATCH_CAPTURE(msg) \
	INTERNAL_CATCH_INFO("CATCH_CAPTURE", #msg " := " << ::Catch::Detail::stringify(msg))
